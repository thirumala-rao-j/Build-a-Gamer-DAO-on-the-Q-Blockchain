...
const { MODULE_IMPLEMENTATION, MODULE_NAME, MAIN_DAO_VOTING_ADDRESS } =
  process.env;
...
async function main() {
  ...
  const addModuleCalldata = getEncodedData(
    "DAORegistry",
    "addProxyContract",
    MODULE_NAME,
  ...
  console.log(
    `Create Proposal to add ${MODULE_NAME} module to the DAO Registry`
  );
  await VotingContract.createProposal(
    "DAORegistry",
    `Adding ${MODULE_NAME} to the DAO Contract Registry`,
    addModuleCalldata,
    {
      from: senderAddress,
    }
